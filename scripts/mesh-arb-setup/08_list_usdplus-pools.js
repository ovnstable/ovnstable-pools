...
async function main() {
    ...
    let count = new BN((await meshFactory.allPairsLength()).toString());
    console.log(`Total pools on mesh: ${count}`);

    let dir = 'pool_infos'
    makeDirIfNeed(dir);

    let poolInfosFileName = `${dir}/meshPools.json`
    let usdPlusPoolInfosFileName = `${dir}/meshPools_usdPlus.json`
    ...
    while (index.gte(zero)) {
        ...
        for (let i = 0; i < 50; i++) {
            promises.push(new Promise((resolve, reject) => {
                (async () => {
                    ...
                    let poolAddress = await meshFactory.allPairs(localIndex);
        ...
        // let poolAddress = await meshFactory.allPairs(index.toString());
        //
        // let pool = await ethers.getContractAt(iUniswapV2PairAbi, poolAddress, wallet);
    ...
    console.log(`--- mesh usdPlus pools:`)
