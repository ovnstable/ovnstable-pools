...
async function main() {
    ...
    async function runTest() {
        ...
        makeDirIfNeed(dir);
        let resultFileName = `${dir}/testShuffleOnStable_10000.json`;
        ...
        await workbook.xlsx.writeFile(`${dir}/testShuffleOnStable_10000.xlsx`);
    ...
    async function testRound(round, tokensToLog) {
        if (round.arbitrage) {
            await testArbitrageOnStable(tokensToLog);
        } else {
            await testShuffleOnStable(round);
        }
    }

    async function testArbitrageOnStable(tokensToLog) {
        // let balances = poolBalances(tokensToLog.stableTokens, names, stablePool, vault);
    ...
    async function testShuffleOnStable(testParams) {
        ...
        let tokenInName = names[tokenIn.address];
        let tokenOutName = names[tokenOut.address];
